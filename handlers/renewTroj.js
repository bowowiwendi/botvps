...
// Fungsi untuk renew Trojan
const renewTroj = (vpsHost, username, exp, quota, limitIp) => {
    return new Promise((resolve, reject) => {
        const command = `printf "${username}\n${exp}\n${quota}\n${limitIp}" | ssh root@${vpsHost} renewtr`;
...
module.exports = (bot, servers) => {
    bot.on('callback_query', async (query) => {
        ...
        try {
            if (data.startsWith('troj_renew_')) {
...
Saldo Anda: Rp ${(admin.balance || 0).toLocaleString()}`);
                ...
                bot.once('message', async (msg) => {
                    ...
                    try {
                        ...
                        // Renew akun
                        const result = await renewTroj(server.host, username, exp, quota, limitIp);
