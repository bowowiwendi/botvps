...
const viewSSHDetails = (vpsHost, username, callback) => {
    ...
    exec(command, (error, stdout, stderr) => {
        if (error) {
            callback(`Error: Gagal mengambil detail SS. Pastikan file /var/www/html/shadowsocks-${username}.txt ada di server.`);
            return;
        }

        // Format hasil menjadi lebih menarik
        const formattedOutput = `🔍 *DETAIL SS* 🔍\n\n` +
                               "```\n" +
                               stdout +
                               "\n```";
...
module.exports = (bot, servers) => {
    bot.on('callback_query', async (query) => {
        ...
        if (data.startsWith('ss_detail_')) {
            ...
            await bot.sendMessage(chatId, 'Masukkan username SS:');
