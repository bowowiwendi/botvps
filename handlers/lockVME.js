...
const lockVME = (vpsHost, username, callback) => {
    const command = `printf "${username}" | ssh root@${vpsHost} lock-vm`;
...
module.exports = (bot, servers) => {
    bot.on('callback_query', async (query) => {
        ...
        if (data.startsWith('vme_lock_')) {
            ...
            await bot.sendMessage(chatId, 'Masukkan username VME yang ingin dikunci:');
            ...
            bot.once('message', async (msg) => {
                ...
                lockVME(server.host, username, (result) => {
